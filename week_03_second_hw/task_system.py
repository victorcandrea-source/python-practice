...
class task_manager:
    ...
    def load_task(self):
        try:
            ...
            exception = custom_exception("File not found")
            print(exception.invalid_input_error())
            self.tasks = [] 
        except json.JSONDecodeError:
            exception = custom_exception("Error decoding JSON")
            print(exception.CantDecodeFileError())
            self.tasks = [] 
    ...
    def create_task(self, title: str, owner: str, description: str = ""):
        if title is None or owner is None:
            exception = custom_exception("Title and owner cannot be None")
            print(self.exception.invalid_input_error())
            return None
        if not self.tasks:
            new_id = 1
        else:
    ...
    def update_task(self, task_id: int, title: str = None, owner: str = None, description: str = None):
        if not task_id:
            exception = custom_exception("Task ID cannot be None")
            print(self.exception.TaskNotFoundError())
            return None
        for t in self.tasks:
            if t.id == task_id:
    ...
    def get_task_by_id(self, task_id: int):
        if not self.tasks:
            exception = custom_exception("No tasks available")
            print(self.exception.TaskNotFoundError())
            return None
    ...
    def change_status(self,task_id: int, new_status: str):
        if not self.tasks:
            exception = custom_exception("Invalid status transition")
            print(self.exception.InvalidStatusTransitionError())
            return None
        for task in self.tasks:
           if task.id == task_id:
                if  not task.status == "CANCELED" or task.status == "BLOCKED":
                    exception = custom_exception(f"Cannot change status from {task.status} to {new_status}")
                    print(self.exception.InvalidStatusTransitionError())
                    return None
                else:
